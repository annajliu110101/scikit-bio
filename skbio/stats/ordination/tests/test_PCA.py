...
from skbio.stats.ordination._principal_coordinate_analysis import pca, _fsvd
...
class TestPCA(TestCase):
    ...
    def test_simple(self):
        results = pca(self.data, method = "svd", dimensions = 50)
    ...
    def test_simple_transposed(self):
        results = pca(self.transposed_data, method = "svd", dimensions = 50)
    ...
    def test_randomized_self(self):
        results = pca(self.data, method = "randomized", dimensions = 50)   
        expected_results = pca(self.data, method = "randomized", dimensions = 50)
        assert_ordination_results_equal(results, expected_results,
                                        ignore_method_names=True,
    ...
    def test_fsvd_self(self):
        results = pca(self.data, method = "fsvd", dimensions = 50)
        expected_results = pca(self.data, method = "fsvd", dimensions = 50)
        assert_ordination_results_equal(results, expected_results,
                                        ignore_method_names=True,
                                        ) 
    def test_eigh(self):
        results = pca(self.data, method = "eigh", dimensions = 50)
        expected_results = pca(self.data, method = "svd", dimensions = 50)
        assert_ordination_results_equal(results, expected_results,
                                        ignore_method_names=True,
                                        )
    def test_eigh_transposed(self):
        results = pca(self.transposed_data, method = "eigh", dimensions = 50)
    ...
    def test_fsvd_transposed(self):
        results = pca(self.transposed_data, method = "fsvd", dimensions = 50)
        expected_results = pca(self.transposed_data, method = "svd", dimensions = 50)
        assert_ordination_results_equal(results, expected_results,
                                        ignore_method_names=True,
                                        )
    def test_fsvd_inplace(self):
        results = pca(self.data, method = "fsvd", dimensions = 50)
        results_inplace = pca(self.data.copy(), method = "fsvd", inplace = True, dimensions = 50)
        expected_results = pca(self.data, method = "svd", dimensions = 50)
    ...
    def test_randomized_method_with_float(self):
        results = pca(self.data, method="randomized", dimensions=0.8,
                       inplace=False, seed=None)
        cumulative_variance = np.cumsum(results.proportion_explained.values)
        self.assertGreaterEqual(cumulative_variance[-1], 0.8)
